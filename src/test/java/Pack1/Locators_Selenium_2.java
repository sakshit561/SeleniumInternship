...
public class Locators_Selenium_2 {
	public static void main(String[] args) throws InterruptedException {
		...
		String pass = passSplit(cd);  
		cd.get("https://rahulshettyacademy.com/locatorspractice/");
		cd.manage().window().maximize();

		cd.findElement(By.xpath("//*[@id=\"inputUsername\"]")).sendKeys("nivesh");
		...
		long num = 7879577398l;
		String str1 = Long.toString(num);
		
		cd.findElement(By.cssSelector("input[type=text]:nth-child(4)")).sendKeys(str1);
		Thread.sleep(2000);
		cd.findElement(By.className("reset-pwd-btn")).click();
		Thread.sleep(2000);
		cd.findElement(By.className("go-to-login-btn")).click();
		cd.findElement(By.xpath("//*[@id=\"inputUsername\"]")).sendKeys(str1);
		
		cd.findElement(By.cssSelector("input[type=password]")).sendKeys(pass);
		Thread.sleep(2000);
		cd.findElement(By.id("chkboxTwo")).click();
		cd.findElement(By.cssSelector("button[type=submit]")).click();
		Thread.sleep(2000);
		String visibleText = cd.findElement(By.tagName("h2")).getText();
		String expectedText = "Hello "+str1 + ",";
		Assert.assertEquals(visibleText, expectedText);
	...
	static String passSplit(WebDriver cd) throws InterruptedException {
		cd.get("https://rahulshettyacademy.com/locatorspractice/");
		cd.manage().window().maximize();
//		cd.findElement(By.xpath("//*[@id=\"inputUsername\"]")).sendKeys("7879577398");
		...
		cd.findElement(By.className("reset-pwd-btn")).click();
		Thread.sleep(2000);
		String fullText = cd.findElement(By.tagName("p")).getText();
		String[] second = fullText.split("'");
		String[] third = second[1].split("'");
		String realPass = third[0];
